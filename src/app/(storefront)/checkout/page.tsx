...
export default async function CheckoutPage() {
  ...
  type ShippingOption = { methodId: string; service: string; label: string; price: number };
  const shippingOptions: ShippingOption[] = [];

  for (const method of shippingMethods) {
    if (method.courier && destinationCityId) {
      const costs = await getShippingCost(destinationCityId, totalWeight, method.courier as CourierCode);
  ...
  return (
    <div className="mx-auto flex max-w-lg flex-col gap-6">
      ...
      <form action={checkout} className="flex flex-col gap-6">
          ...
          {shippingOptions.length === 0 ? (
            ...
            <div className="flex max-h-64 flex-col gap-1 overflow-y-auto">
              {shippingOptions.map((opt, i) => (
                <label key={`${opt.methodId}-${opt.service}-${i}`} className="flex items-center gap-2 py-1">
                  <input
                    type="radio"
                    name="shippingOption"
                    value={`${opt.methodId}:${opt.service}`}
                    defaultChecked={i === 0}
        ...
        <div className="flex flex-col gap-1 rounded-lg border border-black/10 p-4 text-sm dark:border-white/10">
          <div className="flex items-center justify-between">
            ...
            <Link href="/profil?callbackUrl=/checkout" className="underline">Ubah</Link>
          </div>
          <p className="whitespace-pre-line text-black/70 dark:text-white/70">{formatAddress(user)}</p>
        </div>

        <input type="hidden" name="shippingAddress" value={formatAddress(user)} />
        <button type="submit" className="rounded-md bg-black px-5 py-2 text-white dark:bg-white dark:text-black">
