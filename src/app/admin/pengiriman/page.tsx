        ...
        <div className="flex flex-col gap-6">
            ...
            <div>
                ...
                <p className="mt-1 text-xs text-black/40 dark:text-white/40">
                    Metode dengan kurir akan hitung ongkir otomatis via RajaOngkir. Tanpa kurir = ongkir manual.
                </p>
            </div>

            {/* Form tambah */}
            <form action={createShippingMethod} className="flex max-w-lg flex-wrap gap-2">
                <input
                    type="text"
                    name="name"
                    placeholder="Nama metode (opsional untuk kurir)"
                ...
                <input
                    ...
                    min={0}
                    defaultValue={0}
                    className="w-28 rounded-md border border-black/10 px-3 py-2 text-sm dark:border-white/10"
                />
                <button
                    type="submit"
            ...
            {methods.length === 0 ? (
                <p className="text-black/60 dark:text-white/60">Belum ada metode pengiriman.</p>
            ) : (
                <table className="w-full max-w-3xl text-sm">
                    <thead>
                        <tr className="border-b border-black/10 text-left dark:border-white/10">
                            <th className="py-2">Nama</th>
                            <th className="py-2">Kurir</th>
                            ...
                            <tr key={method.id} className="border-b border-black/5 dark:border-white/5">
                                ...
                                <td className="py-2">
                                    {method.courier ? (
                                        ...
                                        <span className="text-black/30 dark:text-white/30">Manual</span>
                                    )}
                                </td>
                                <td className="py-2">
                                    {method.courier ? (
                                        <span className="text-xs text-black/40 dark:text-white/40">Auto via API</span>
                                    ) : (
                                        <form action={updateShippingMethod} className="flex items-center gap-1">
                                            <input type="hidden" name="id" value={method.id} />
                                            <input type="hidden" name="name" value={method.name} />
                                            <input type="hidden" name="isActive" value={String(method.isActive)} />
                                            <span>Rp</span>
                                            <input
                                                type="number"
                                                name="price"
                                                ...
                                                min={0}
                                                className="w-20 rounded border border-transparent px-1 py-0.5 hover:border-black/10 focus:border-black/20 dark:hover:border-white/10 dark:focus:border-white/20"
                                            />
                                ...
                                <td className="py-2">
                                    <form action={updateShippingMethod}>
                                        <input type="hidden" name="id" value={method.id} />
                                        <input type="hidden" name="name" value={method.name} />
                                        <input type="hidden" name="price" value={method.price} />
                                        <input type="hidden" name="courier" value={method.courier ?? ""} />
                                        <button
                                            type="submit"
                                            name="isActive"
                                            value={method.isActive ? "false" : "true"}
                                            className={`rounded-full px-2 py-0.5 text-xs font-medium ${method.isActive
