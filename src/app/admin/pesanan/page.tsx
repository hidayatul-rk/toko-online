  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-xl font-semibold">Pesanan ({total})</h1>
        <ExportCsvButton />
      ...
      <div className="flex flex-wrap gap-2">
        <Link
          href={filterHref("ALL")}
          className={`rounded-full border px-3 py-1 text-xs ${!status || status === "ALL"
        ...
        {STATUS_OPTIONS.map((s) => (
          <Link
            key={s}
            href={filterHref(s)}
            className={`rounded-full border px-3 py-1 text-xs ${status === s
              ? "border-black bg-black text-white dark:border-white dark:bg-white dark:text-black"
              : "border-black/10 dark:border-white/10"
              }`}
          >
      ...
      <div className="flex flex-col gap-4">
        {orders.map((order) => (
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <ToastForm action={updateOrderStatus} className="flex items-center gap-2">
            ...
            <div className="mt-2 flex justify-between border-t border-black/10 pt-2 text-sm dark:border-white/10">
              <span className="text-black/60 dark:text-white/60" title={order.shippingAddress}>
                {order.shippingAddress.slice(0, 50)}{order.shippingAddress.length > 50 ? "..." : ""}
              </span>
        ...
        {orders.length === 0 && (
          <p className="text-black/60 dark:text-white/60">
            {q ? "Pesanan tidak ditemukan." : "Belum ada pesanan."}
          </p>
        )}
      </div>
